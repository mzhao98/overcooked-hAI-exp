...
// import {loadTwoGraphModel} from '@tensorflow/tfjs-converter';
...
export default function getOvercookedPolicy(model_type, layout_name, playerIndex) {
    ...
    // const modelPromise2 = loadGraphModel('../static/assets/' + model_type + '_' + layout_name + '_agent/model.json');

    return modelPromise.then(function (model) {
		return new Promise(function(resolve, reject) {
			resolve(function (state, game) {
			let action_tensor_1 = model.execute(preprocessState(state, game, playerIndex));
			let action_probs_1 = action_tensor_1.arraySync()[0];
			// let action_1 = Action.INDEX_TO_ACTION[argmax(action_probs_1)];
